...
3. **Supersession pattern** (HIGH) — Is there a date-versioned file in the same directory with a newer date? The newer file `supersedes` the older one.
...
- **Apply the content test to every proposal.** A connection earns its place only when the context says something a reader could not infer from where the two notes already sit. Reject your own proposal if the context would be a placeholder ("Parent project design document", "Related to this project"), a paraphrase of the target's title, or a bare category statement ("both concern data centers"). **Self-check before returning:** re-read each context and ask what the target note's reader actually gains. Drop the ones with no answer. (This self-check has never once been sufficient — a code-side gate is authoritative — but it reduces wasted proposals.)
  - **Same-directory connections are allowed** and judged by the same content test (changed 2026-07-25, replacing a blanket ban plus a "project hub carve-out"). The ban rejected genuine sibling edges while its own carve-out generated 111 contentless "Parent project design document" links, and it protected no score: orphan detection and coverage both count cross-directory links only. **Still prefer a cross-directory target** when both are defensible — that is what connects PARA silos — but do not suppress a real relationship because two notes share a folder, and never propose a hub link merely to make an orphan look connected.
