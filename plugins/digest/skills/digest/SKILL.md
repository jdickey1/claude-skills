...
- **Thread**: `conversation_count` > 0 — with xurl, walk `conversation_id` via `/2/tweets/search/recent` (exact query and the 7-day-window caveat are in Gotchas, "X threads live in replies"); for posts older than 7 days that walk silently returns nothing, so use twitter-cli, which auto-fetches the full thread in one call; dev-browser requires scrolling to capture self-replies
...
Pass: xurl, twitter-cli, or dev-browser used as fetch method; thread/article content captured — with xurl, that means the `conversation_id` walk in Gotchas ("X threads live in replies"), not just the parent fetch; for posts older than ~7 days, that walk was skipped in favor of twitter-cli or dev-browser
Fail: Post dismissed as "engagement bait" or "missing promised content" without attempting to fetch replies, OR a `result_count: 0` from `/2/tweets/search/recent` on a post older than 7 days was read as "no thread" rather than an out-of-window miss
...
- **X threads live in replies — and the xurl walk only reaches back 7 days** — Most X "threads" are a hook tweet with the real content in self-replies, and the syndication API only returns the parent tweet. Check `created_at` before choosing a tool. Within ~7 days, walk the thread with xurl: `xurl "/2/tweets/search/recent?query=conversation_id%3A${TWEET_ID}%20from%3A${USERNAME}&tweet.fields=note_tweet,text&max_results=20"`. URL-encode the space and colons — an unencoded query returns a parse error, not results. Filter to `from:` the author to separate self-replies (the thread body) from the crowd's replies. **Outside the window the endpoint returns `{"meta":{"result_count":0}}` with HTTP 200 — no error, no warning, indistinguishable from a genuinely reply-less post.** Verified: post `2045133175320707513` (2026-04-17) walks to zero while the syndication API reports `conversation_count: 27`. So never read `result_count: 0` on an older post as "no thread" — skip the walk entirely and use twitter-cli, which auto-fetches the full thread in one call, then dev-browser scrolling. `/2/tweets/search/all` would lift the window but returns 403 on this OAuth 2.0 User Context tier; there is no upgrade path.
