...
version: 1.0.0
effort: high
expires_when: "One model instance can critique, rewrite, and judge its own content in a single context without sycophancy or anchoring, retiring the multi-agent context-isolation apparatus; the critic→author→synthesizer→judge workflow shape, the four role prompts, and the output contract stay"
---
